...
if (!$search) {
  ...
  $request   = "SELECT SQL_CALC_FOUND_ROWS * FROM shoutbox  WHERE setting != '-1' ORDER BY id DESC {$limit}";
  if ($amode == 1 and $_GET['sa'] == 1)
    $request = "SELECT * FROM shoutbox where setting != '-1' ORDER BY id DESC";
  if (3 == $amode)
    $request = "SELECT SQL_CALC_FOUND_ROWS * FROM shoutbox where setting = '-1' ORDER BY id DESC {$limit}";
  if ($amode == 3 and $_GET['sa'] == 1)
    $request = "SELECT * FROM shoutbox where setting = '-1' ORDER BY id DESC";
  $result           = mysql_query($request);
  $max_page         = 1;
  $pagination_links = "";
...
while ($row = mysql_fetch_object($result)) {
  ...
  $setting   = $row->setting;
  $out .= message2html($msg,$user,$timestamp,$setting,$id,$amode);
  continue;
  //print "<p> ($setting , $amode ) </p>";
  if ($amode != 1 and $amode!=3 and $setting == 1) {
...
if (!$search && !$json) {
  ...
  if ($GET_['sa'] != 1 and $amode != 1 and $amode !=3)
    $out .= '<p style="text-align:center"><a href="/exp4/shoutbox2.php?sa=1">Show all Messages</a></p>';
  if ($GET_['sa'] != 1 and $amode == 3)
    $out .= '<p style="text-align:center"><a href="/exp4/shoutbox2.php?admin=1&deleted=1&sa=1">Show all Messages</a></p>';
} //!$search && !$json
if ($json) {
