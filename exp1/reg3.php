...
if($step==4)
{
	...
	if($pgame==-1)
	{
		...
		{	
			...
			$dtarr=array($lastgame,date("D, d M Y H:i T", strtotime($lastgame)));
		}
...
{	
	...
	for($i3=1;$i3<=10;$i3++)
	{
		...
		if($c==0 and ($step==1 or $step==2 or $step==3 or $step==4)) print "<tr><td>$i3</td><td colspan=3></td></tr>";
		if($c==0) continue;
		print "\n<tr><td>$i3.</td><td>";
		...
		if($step==1 or $step==2 or $step==3 or $step==4)
		{
			...
			if($step==1 and $c==1 ) print "<span style=\"color:#2E8B57\">$ts2</span></td><td>";
